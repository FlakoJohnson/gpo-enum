...
// DialTCPFunc, if non-nil, replaces net.DialTimeout for TCP connections to the
// KDC. The function receives the raw host:port address (no prior DNS resolution
// — the proxy handles that) and a connect timeout. Callers set this to route
// KDC traffic through a SOCKS5 proxy.
var DialTCPFunc func(addr string, timeout time.Duration) (net.Conn, error)
...
func dialSendTCP(kdcs map[int]string, b []byte) ([]byte, error) {
	...
	for i := 1; i <= len(kdcs); i++ {
		...
		if DialTCPFunc != nil {
			conn, err = DialTCPFunc(kdcs[i], 5*time.Second)
		} else {
			tcpAddr, resolveErr := net.ResolveTCPAddr("tcp", kdcs[i])
			if resolveErr != nil {
				errs = append(errs, fmt.Sprintf("error resolving KDC address: %v", resolveErr))
				continue
			}
